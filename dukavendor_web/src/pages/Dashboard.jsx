...
function Dashboard({ children }) {
  ...
  return (
    <div className="dashboard">
      <nav className={`side-nav ${isNavMinimized ? 'minimized' : 'side-nav'}`}>
        <div className='nav-items'>
          <img src={DarkLogo} />
        </div>
        <img src={Minimize} onClick={toggleNav} style={{ position: "absolute", right: '-38px', paddingTop: '20px' }} />
        <div className='nav-items'>
        </div>
        <div className='nav-box' >
          <div className='nav-items' onClick={() => navigate('/')}>
            <svg className='' width="42" height="47" viewBox="0 0 42 47" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <span className={`${isNavMinimized ? 'hide-text' : ''}`} style={{ paddingLeft: "15px" }}><p className={location.pathname === '/' ? 'p' : ''}>Dashoboard</p></span>
            {location.pathname === '/' ? (
              <img src={Line} style={{ position: 'absolute', right: '0' }} />
            ) : (
              ''
            )}
          </div>
          <div className='nav-items' onClick={() => navigate('/orders')}>
            ...
            <span className={`${isNavMinimized ? 'hide-text' : ''}`} style={{ paddingLeft: "15px" }}><p className={location.pathname === '/orders' ? 'p' : ''}>Orders</p></span>
          ...
          <div className='nav-items' onClick={() => navigate('/products')}>
            ...
            <span className={`${isNavMinimized ? 'hide-text' : ''}`} style={{ paddingLeft: "15px" }}><p className={location.pathname === '/products' ? 'p' : ''}>Products</p></span>
          ...
          <div className='nav-items' onClick={() => navigate('/wallet')}>
            ...
            <span className={`${isNavMinimized ? 'hide-text' : ''}`} style={{ paddingLeft: "15px" }}><p className={location.pathname === '/wallet' ? 'p' : ''}>Wallet</p></span>
          ...
          <div style={{ paddingTop: '100px' }}>
            <div className='nav-items' onClick={() => navigate('/sign-in')}>
              <img src={logOut} />
              <span className={`${isNavMinimized ? 'hide-text' : ''}`} style={{ paddingLeft: "15px" }}>Log out</span>
            </div>
          </div>
        </div>
      </nav>
      <main className="main">
        <header className='header'>
          ...
          <div className='profile-section'>
            <img src={Bell} style={{ paddingRight: "50px" }} />
            <img src={Profile} style={{ paddingRight: "50px" }} />
            <img src={Dropdown} />
          </div>
        </header>
        {children}
